...
export function TagPicker({ noteId, selectedTags, onClose }: TagPickerProps) {
  ...
  return (
    <div className="absolute left-0 right-0 mt-2 bg-white rounded-lg shadow-lg border border-gray-200 p-3 z-20">
      {/* Existing tags */}
      {tags.length > 0 && (
        <div className="mb-3">
          ...
          <div className="flex gap-2 mt-2 flex-wrap">
            {DEFAULT_COLORS.map(color => (
              <button
                ...
                className={`w-6 h-6 rounded-full ${selectedColor === color ? 'ring-2 ring-offset-1 ring-gray-400' : ''}`}
            ...
            <button
              ...
              className={`w-6 h-6 rounded-full bg-gray-400 ${!selectedColor ? 'ring-2 ring-offset-1 ring-gray-400' : ''}`}
              title="Auto"
            />
          </div>
        )}
